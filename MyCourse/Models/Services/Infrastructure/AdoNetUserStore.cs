...
namespace MyCourse.Models.Services.Infrastructure
{
        ...
        IUserConfirmation<ApplicationUser>
    {
        ...
        {
            int affectedRows = await db.CommandAsync($"DELETE FROM AspNetUsers WHERE Id={user.Id}", cancellationToken);
            if (affectedRows > 0)
            {
                return IdentityResult.Success;
            }
